...
PITCHERS - Starters (GS > 0):
  - Age <= 22, K/9 >= 10.0, BB/9 <= 3.5, ERA <= 4.00, 10+ IP

PITCHERS - Elite Relievers (GS = 0, Mason Miller tier):
  - Age <= 22, K/9 >= 13.0, BB/9 <= 2.5, ERA <= 3.00, 10+ IP

All pitcher results sorted by age ascending.
"""
...
# Elite reliever filters (Mason Miller tier)
MIN_IP_RP      = 10.0
MAX_ERA_RP     = 3.00
MIN_K9_RP      = 13.0
MAX_BB9_RP     = 2.5
        ...
        for entry in roster:
            ...
            if age <= MAX_AGE_PIT:
                ...
                if stat:
                    ...
                    if is_starter and ip >= MIN_IP_SP and era <= MAX_ERA_SP and k9 >= MIN_K9_SP and bb9 <= MAX_BB9_SP:
                        qualifies = True
                        role_label = "SP"
                    elif not is_starter and ip >= MIN_IP_RP and era <= MAX_ERA_RP and k9 >= MIN_K9_RP and bb9 <= MAX_BB9_RP:
                        qualifies = True
                        role_label = "RP"
...
def build_pitcher_field(p):
    role_icon = "🔵" if p["role"] == "SP" else "⚡"
    return {
        "name": role_icon + " " + p["name"] + " (" + str(p["age"]) + ") · " + p["role"] + " · " + p["level"],
        "value": "\n".join([
...
def post_to_discord(webhook_url, all_hitters, all_pitchers):
    ...
    # Pitchers: sorted by age ascending (youngest first)
    all_pitchers.sort(key=lambda x: x["age"])
    if all_pitchers:
        sp_count = sum(1 for p in all_pitchers if p["role"] == "SP")
        rp_count = sum(1 for p in all_pitchers if p["role"] == "RP")
        desc = (
            "**" + str(len(all_pitchers)) + " pitchers** · " +
            str(sp_count) + " SP / " + str(rp_count) + " elite RP · Sorted by age\n" +
            "SP: Age <=22 · K/9 >=10.0 · BB/9 <=3.5 · ERA <=4.00 · 10+ IP\n" +
            "RP: Age <=22 · K/9 >=13.0 · BB/9 <=2.5 · ERA <=3.00 · 10+ IP"
        )
        post_section(webhook_url,
